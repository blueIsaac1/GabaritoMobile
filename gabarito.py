...
# Função para processar e extrair as respostas da imagem do gabarito
...
# Função para comparar as respostas do aluno com o gabarito correto do professor
...
def generate_report(results, output_file):
    with open(output_file, mode='a', newline='', encoding='utf-8') as file:
        ...
        for nome_aluno, pontuacao, nota_trabalho, nota_final in results:
            writer.writerow([nome_aluno, pontuacao, nota_trabalho, nota_final])
...
student_image_paths = glob.glob("./gabarito_flask/uploads/*.jpg")
resultados_alunos = []

# Processa cada aluno individualmente
for aluno_image_path in student_image_paths:
    resultado = process_student(gabarito_professor, aluno_image_path)
    if resultado:
